...
- **FR-ELEM-3** 流量连接器:属性 id/type:"flow"/fromId/toId/formula(支持 `数值 [单位]` 标注)/units(自动派生为目标存量 units/时间单位,只读)/isVariable(true `▼`/false `○`);方向由 fromId→toId 表达,不设极性字段;公式以 `@<uuid>` 引用存量(stockId),编辑器显 name,重命名只改 name 不动 id;Bresenham 网格寻路 + 端点箭头 + 端口吸附。(重名软警告源消失,见 FR-ELEM-5 命名机制)
- **FR-ELEM-4** 连接端口:每元素周边预定义连接点;创建连线自动吸附端口;拖拽元素端口位置随更新。端口吸附非端点未连告警;flow 端点未连检测见 FR-ELEM-6。
- **FR-ELEM-5** 图元命名机制:图元 name 全局唯一(跨 stock/cloud/flow);新建序号按类型递增(stock_1/cloud_1/flow_1,默认名格式留 i18n);改名撞名即时禁止(非软警告);删除后序号不复用(继续递增);粘贴图元视为新建序号递增;id=uuid 不变(公式 @uuid 引用不断,line 394 保留)。覆盖 stock/cloud/flow 全部图元。
- **FR-ELEM-6** flow 端点完整性:flow fromId/toId 两端须连已存在图元(stock/cloud);创建中不报/放下后报(设置错误,状态栏 ⚠N 不阻断);源/汇端 flow 连 cloud 算已连;与 1a.4 E10 孤立 cloud 对称;删图元致 flow 端点失连同此检测(4.2 endpoint RI 级联删 flow 后不再产生此源,见 Story 4.2)。
...
- **FR-SIM-7** 量纲一致性校验(MVP):编译期对每流量公式遍历 AST 逐子表达式推导单位,比对目标单位(目标存量 units/时间单位);一致通过,不一致软警告(红色高亮+预期/推导单位,不阻断运行);常数单位标注参与推导;无 units 视无量纲(混合软警告=量纲不一致子情况,依赖 1b 引擎,1a.12 不独立实现);校验在公式编辑时实时进行(FR-UI-2 扩展)。
...
- **FR-UI-3** 底部状态栏:模拟时间计数器+图元计数+在线用户数+头像堆栈+FPS(Debug)+连接状态+设置问题概要(渐进暴露:⚠N count L1 默认可见 + popover 详情 L2 渐显,如 `⚠ X 错误 · Y 软警告`;错误=flow 端点未连(必修致不生效)/软警告=量纲/混合/孤立/parallel(可忽略);popover 清单项点 -> 跳画布对应图元 + 属性面板展开,修复即消;不分组/筛选/排序(预留口))。
...
- **FR-UI-7** PromptPanel 容器:四 tab["!" / 里程碑 / 源/汇 / 存量] + 收起态胶囊(不展示内容,横向 tab 名 + ⏏️ 展开键);"!" tab 收未答 confirm(仪式性不可逆操作,❕红渲染,一次性交互响应后转已解决纯展示)+ 运算错误 alert(⚠橙渲染)+ 非负钳制通知(1b 接入,不计角标 count);收起态"!"有未答 confirm -> tab 名红光闪烁角标;点 ⏏️/tab 名:有未答切"!"/无未答显上次活动或该 tab;里程碑 tab 独占里程碑回顾(★已达成/☆未达成,纯展示无操作),结构在 1a.12 内容 defer 游戏化中心(Epic 5);设置错误(状态性,持续 till 修复)归状态栏 ⚠N(FR-UI-3 修订)+ tab 行内,运算错误(时序性,运行时事件)归"!"tab alert;错误源跨 epic--1a.12 独立交付 flow 端点未连/孤立/parallel,量纲(含混合)1b 接入,公式悬空 4.2 接入,未实现源空占位(非 UI stub);cap 全保留 + 兜底 1000 + 滚动,"!"额外 toast 4s 自动移除。
- **FR-UI-8** 存量 tab:stock 状态表,列 名称|变化值|单位|问题;变化值=净流量(Σ接入源 flow − Σ接入汇 flow,带符号纯数字不附单位);单位列显存量单位;行首标识 ⚫负/⚪正/☯零;单击行选中图元 -> 画布定位 + 右栏属性面板(FR-UI-2,1a.8)展开;cap 全保留 + 兜底 1000 + 滚动。
- **FR-UI-9** 源/汇 tab:cloud 状态表,列 名称|连接|流量|问题;流量=流经该 cloud 的 flow 当前速率(多条取代数和);行首标识 ☁源(白渲染)/☁汇(黑渲染);单击选中 -> 画布跳转 + 右栏属性面板展开;cap 全保留 + 兜底 1000 + 滚动。
...
- **FR-GAME-4** 游戏化中心(MVP):表盘 + 等级机制 + 徽章墙;等级触发挂 SD 建模行为(如"完成一个模型搭建升 1 level",完成语义待 CS);表盘归游戏化中心(非状态栏非 PromptPanel);里程碑 tab 回顾内容(FR-UI-7)在本 story 实现(结构在 1a.12 重构 story defer 到此);与 FR-GAME-2 行为徽章关系(徽章解锁计入等级?)留 CS;原 5.3 toast 即时反馈是否保留留 CS。
...
FR-ELEM-5: Epic 1a - 图元命名机制(全局唯一+序号递增,重名源消失)
FR-ELEM-6: Epic 1a - flow 端点完整性(两端须连已存在图元,放下后报 ⚠N)
...
FR-UI-3: Epic 1a - 底部状态栏(时间/图元数/在线/FPS/设置问题概要)
...
**FRs covered:** FR-CANVAS-1, FR-CANVAS-2, FR-CANVAS-3, FR-CANVAS-4, FR-CANVAS-5, FR-ELEM-1, FR-ELEM-2, FR-ELEM-3, FR-ELEM-4, FR-ELEM-5, FR-ELEM-6, FR-UI-1, FR-UI-2, FR-UI-3, FR-UI-4, FR-UI-5, FR-UI-7, FR-UI-8, FR-UI-9(共 19)
...
**And** SQLite WAL 备份原语(AR#15:raw cp 对 WAL 模式 SQLite 捕获不一致状态,改用三选一可验证原语 — VACUUM INTO / sqlite3 .backup API / checkpoint+cp(main+-wal);恢复测试验证可还原,F18-sqlite-backup 从 defer 提升为 MVP 验收,B1 决策:单节点无备份=数据永久丢失,MVP 级风险;备份治全丢不治定点回滚,A3 见 PRD 后续候选)

**Acceptance Criteria(边界 guard 段 — E7 Float64 精度极限 clamp):**

**Given** 画布以 Float64 世界坐标系为底(平移/缩放)
**When** 用户平移到 Float64 精度极限(世界坐标绝对值趋 Number.MAX_SAFE_INTEGER 量级)
**Then** clamp 世界坐标范围到安全区间(实现期标定,如 ±1e15)或 warn 精度丢失
**And** 防精度退化致图元错位/吸附失准(1a.2 渲染基座坐标稳定)

**Acceptance Criteria(NFR 段 — AR#5 PERF-4 加载时间):**

**Given** 1a.1 应用骨架 + 1b.1 Wasm 首屏懒加载(仿真首次触发才加载)
**When** 用户访问应用 URL 加载 1000 图元画板
**Then** 加载时间 ≤ 2 秒(NFR-PERF-4)— 1a 建模/渲染不依赖 Wasm(首屏不阻塞),Wasm 产物经 wasm-opt+gzip+内容哈希长缓存

**Acceptance Criteria(边界 guard 段 — AR#12 板加载 UX + 全局错误边界):**

**Given** 用户访问应用 URL 加载画板(1a.1,PERF-4 ≤2s)
**When** 加载期间 / 运行时未捕获错误
...
**Acceptance Criteria(边界 guard 段 — E9 stock 尺寸合法性):**

**Given** 用户创建存量(stock)
**When** 填入 width/height
**Then** validate width>0 且 height>0 且为数值(≤0 或非数 reject 或夹回默认)
**And** 防零/负尺寸致渲染退化(方框退化)或空间索引 R 树插入异常
...
**Acceptance Criteria(边界 guard 段 — E3/E10/E11 流量端点边界):**

**Given** 用户创建流量(flow)
**When** fromId==toId(self-loop,E3)
**Then** 创建时 reject(校验失败提示,防自环致代数环/守恒破坏)— 创建侧 1a.4;粘贴侧 4.3 同拒
**And** 无 flow 附着的 orphan cloud(E10):allow orphan(边界元素无限容量,语义由 flow 涌现非必有 flow),persist 时 warn(状态栏提示"N 个孤立 cloud")— 1a.4 创建侧 / 4.x 持久化侧
**And** 两 parallel flows 同 fromId/toId(E11):allow(允许并行流量,物质可分流)+ warn duplicate(状态栏软警告)— 1a.4 创建侧 / 4.x 侧(parallel 非重名;命名机制 FR-ELEM-5 下重名不可能,parallel 仍 allow + warn duplicate 物质分流)

**Acceptance Criteria(边界 guard 段 — AR#12 空画板空态):**

**Given** 新建/打开空画板(1a.4,无图元)
**When** 画板为空
**Then** 显空态引导(占位文案/图示引导用户建首个图元,非空白画布无提示)
**And** 空态不阻塞操作(用户可立即开始建图元);与 B10 空模型禁仿真触发一致(空可建图元但不可跑仿真)
...
**Acceptance Criteria(NFR 段 — AR#5 PERF-1 千图元帧率):**

**Given** 1a.5 空间索引+视口剔除就绪
**When** 1000 图元画布渲染稳态
**Then** 帧率 ≥ 60 FPS(NFR-PERF-1)— 静态渲染口径;行进动画 60FPS 在 5.1 补(B-perf-1)

**Acceptance Criteria(边界 guard 段 — B-obs-1 前端性能 RUM 采集):**

**Given** 前端性能 KPI(NFR-PERF-1~5:60FPS/协作延迟/加载/内存)
**When** 线上客户端运行
**Then** 前端 RUM 采集上报(60FPS/加载时/内存/协作延迟客户端侧),运维可视线上客户端达标(非仅后端 stdout)
**And** MVP 口径:实施期 RUM 接入(轻量上报,P95 聚合非每帧),或声明延后(显式声明非静默漏)
...
**Acceptance Criteria(边界 guard 段 — E8 空画板 minimap):**

**Given** 画板无图元(zero 图元)
**When** 渲染小地图
**Then** 渲染 empty minimap placeholder(如居中提示或空白框)非空白崩溃
**And** placeholder 随图元创建消失(首个图元入板即切正常投影)
...
**And** 删除按钮(编辑菜单 Delete / 选中图元后 Delete 键)在 1a 激活:plain properties 本地删除选中图元(stock/cloud/flow,1a 单人无 CRDT);删除存量时的级联 RI 与 formula-reference dangling 规则在 4.2 CRDT 事务化时补(受控 rework,类 1a.7→1a.9 i18n 抽 key;AR-1 修订:无删除则单人建模闭环不成立)。1a.12 阶段(4.2 前)plain properties 本地删除无 RI,删存量致 flow 端点失连由 FR-ELEM-6 检测报(状态栏 ⚠N);4.2 endpoint RI 级联删 flow 后不再产生 flow 端点未连源,FR-ELEM-6 检测逻辑保留作防御(可移除)
**And** 工具栏文案写死中文(1a.9 抽 i18n key,接受返工)

**Acceptance Criteria(状态栏):**

**When** 实现底部状态栏
**Then** 含 模拟时间计数器 + 图元计数 + 在线用户数 + 头像堆栈 + FPS(Debug)+ 连接状态 + 设置问题概要(⚠N count L1 默认可见 + popover 详情 L2 渐显,如 `⚠ X 错误 · Y 软警告`,见 FR-UI-3)
**And** 设置问题概要 popover 清单项点 -> 跳画布对应图元 + 属性面板展开(L2,见 FR-UI-3)
**And** 在线用户数/FPS/连接状态在 1a 单人模式下显示占位(单人为 1 / 连接状态本地)

**Acceptance Criteria(边界 guard 段 — AR#11 键盘可达 a11y):**

**Given** 工具栏/状态栏(1a.7)
**When** 键盘操作
**Then** 全功能键盘可达(Tab 焦点流/快捷键/Delete 删除/方向键移动图元),非仅鼠标
**And** 焦点可见(focus ring 非隐藏),Tab 顺序合逻辑

**Acceptance Criteria(边界 guard 段 — AR#11 色盲安全+对比度 a11y):**

**Given** 工具栏/状态栏配色(1a.7,VRAM 色板 1a.2)
**When** 配色选择
**Then** 色盲安全(非仅色编码信息,辅以图标/文字/形状区分,如状态用图标+文字非纯色)+ 文字对比度 WCAG AA(≥4.5:1 正文)
**And** 防红绿/红蓝色盲用户丢信息;VRAM 等宽网格配色已服从美学,加色盲安全约束不破美学
...
**And** 1a.8 scope:交付功能逻辑层(公式编辑/校验/量纲 stub)+ 最简 UI 容器;容器层 tab 化/新 tab/错误二分归宿在 1a.12 重构(见 Story 1a.12),功能逻辑复用 UI 不白做

**Acceptance Criteria(边界 guard 段 — AR#11 屏幕阅读器 a11y):**

**Given** 属性面板/公式编辑器(1a.8)
**When** 屏幕阅读器(NVDA/VoiceOver)访问
**Then** ARIA label 标注图元属性/公式编辑器语义(字段名/校验错误状态),非仅视觉
**And** 校验错误(红色高亮)须 aria-live 朗读(非仅视觉红高亮)

#### Story 1a.11: 图元命名机制

As a 单人建模者,
I want 图元 name 全局唯一 + 按类型序号自增(不可重名),
So that 重命名/粘贴不产生歧义,公式引用稳定不断。

**Acceptance Criteria(命名唯一性,细节留 CS):**

**Given** 1a.8 属性面板就绪
**When** 新建/改名/粘贴/删除图元
**Then** 图元 name 全局唯一(跨 stock/cloud/flow,FR-ELEM-5)
**And** 新建序号按类型递增(stock_1/cloud_1/flow_1,默认名格式留 1a.9 i18n)
**And** 改名撞名即时禁止(非软警告)
**And** 删除后序号不复用(继续递增)
**And** 粘贴图元视为新建序号递增
**And** id=uuid 不变(公式 @uuid 引用不断)
...
**And** 运算错误归"!"tab alert
**And** 错误源跨 epic(检查点5 选项C):1a.12 独立交付 flow 端点未连(FR-ELEM-6)/孤立 cloud/parallel flow;量纲(含混合)1b 接入占位空;公式悬空 4.2 接入占位空;未实现源空占位(非 UI stub)
...
**Acceptance Criteria(边界 guard 段 — E27 missing key fallback):**

**Given** i18n 运行时切换中/英(1a.9)
**When** 某 key 缺翻译(missing translation key)
**Then** fallback English(英文兜底显示,非空串/非 key 名裸露/非崩)
**And** missing key 在 console 记 warning(开发期补译),生产兜底不影响用户操作
...
**Acceptance Criteria(边界 guard 段 — AR#5 SUCCESS-1 首次建模闭环):**

**Given** Epic 1a 全 story 就绪(1a.1-1a.10 单人建模闭环)
**When** 新用户首次建模
**Then** 从空白到首个可仿真模型 ≤5min(NFR-SUCCESS-1),含建图元/连流量/写公式/配时间单位/跑仿真
**And** 测量:新用户可用性测试(无引导)中位数 ≤5min 达闭环;1a 验收口径
...
> **e2e canvas-click 基础设施 defer(1a.8 D4, 2026-07-13 归属裁定)**: CanvasView 纯 WebGL canvas(无 DOM overlay), e2e canvas-click 基础设施(DOM overlay 层 / spec 重写用坐标 click + 像素断言)1a.8 裁定 defer 归 1b, 1b e2e 集成 story 补(具体 story 待 1b CS), 作 1b 仿真闭环 e2e 验证前置;详见 _bmad-output/implementation-artifacts/deferred-work.md 1a.8 D4.
...
**Acceptance Criteria(边界 guard 段 — E23 worker crash-loop 限):**

**Given** Wasm worker trap/panic(1b.1,AD-5 隔离)
**When** worker 连续 crash-loop(短时多次重启仍 trap)
**Then** crash 计数超阈值(如 5 次/30s)→ halt sim + 通知用户(显式“内核崩溃已暂停仿真”非静默无限重启)
**And** 防 worker 无限重启崩浏览器主线程;halt 后用户可手动重置(非自动重试)

**Acceptance Criteria(边界 guard 段 — AR#5 PERF-5 Wasm 内存预算):**

**Given** Wasm 内核 + Worker 运行(1b.1)
**When** 仿真规模增长(存量/公式数)
**Then** Wasm 内存 ≤50MB(NFR-PERF-5),超限触发 FR-SIM-6 熔断或量级降级非 OOM 崩
**And** 测量:开发者工具内存快照验证 100 存量模型 Wasm heap ≤50MB
...
**Acceptance Criteria(边界 guard 段 — E4 DELAY 非正延迟拒收):**

**Given** 公式含 DELAY 函数 DELAY(X,delay)
**When** 编译期转串联隐式存量
**Then** delay≤0 时 reject(校验失败提示,防零/负延迟致串联存量退化或无意义)
**And** delay>0 正常转串联隐式存量(主 AC 不变)
...
**Acceptance Criteria(NFR 段 — AR#5 PERF-7 多客户端数值偏差):**

**Given** 房主权威仿真(AD-1,仅房主跑)广播整步收敛值(3.4)
**When** 多客户端接收渲染(不回写不本地计算)
**Then** 多客户端间模拟数值偏差 < 0.001%(NFR-PERF-7)— 偏差源头仅浮点渲染舍入非求解器分叉(房主权威单一收敛值)
...
**Acceptance Criteria(边界 guard 段 — E5 LU 奇异降级):**

**Given** 牛顿迭代含 LU 分解线性求解(faer,1b.3 主循环)
**When** 雅可比 rank-deficient 致 LU 奇异(矩阵不可逆)
**Then** catch LU singular → 触发 FR-SIM-8 降级链(后向欧拉→显式试探步);降级链仍失败则 [SYSTEM HALTED] 暂停
**And** 不抛未捕获异常崩 worker(LU 奇异是可恢复数值边界非致命错,与 AD-5 worker 隔离边界一致)
...
**Acceptance Criteria(边界 guard 段 — E2 负初值创建拒收):**

**Given** stock 创建(1a.3)且 allowNegative=false
**When** 用户填入 initialValue<0
**Then** 创建时 reject(校验失败提示,防负初值进入仿真域绕过非负约束)
**And** allowNegative=true 的存量允许负 initialValue(显式 opt-in,1b.4 速率级钳制不激活)
...
**Acceptance Criteria(边界 guard 段 — E1 threshold epsilon floor):**

**Given** 全模型存量 initialValue=0(或均极小)
**When** 计算溢出熔断阈值 threshold=max(maxAbsInitial,epsilon)×1e6
**Then** epsilon floor(默认 1.0,实现期标定)防 maxAbsInitial=0 致 threshold=0 误熔断
**And** 零初值模型不因 threshold=0 空触发熔断(主 AC 阈值逻辑不变,仅 floor 防退化)
...
**Acceptance Criteria(边界 guard 段 — E6 已最简模型 Simplify disable):**

**Given** 模型已无可用降级组合(无 DELAY 可拆/无可变流量可常数化等)
**When** 降级链失败弹"简化模型以继续"单按钮
**Then** 检测无降级组合时 disable 按钮 + toast"模型已最简,无可降级组合"
**And** 防用户点无效 Simplify 反复触发空降级
...
**Acceptance Criteria(边界 guard 段 — E26 时间单位切换暂停仿真):**

**Given** 仿真运行中(1b.8 播放态)+ 用户改模型时间单位(1a.10)
**When** 时间单位切换(年/月/日/时/分/秒)
**Then** 暂停 sim(切单位致量纲重校验/时间计量基准变,跑动切单位致结果错乱)
**And** 切换后需用户手动重播(非自动续跑);量纲重校验(1a.10)未就绪时阻断重播提示
...
As a 协作建模者,
I want 用 GitHub/Google OAuth 登录并获得跨设备 session,
So that 我的画板与徽章绑账号而非绑浏览器匿名态。
...
**Acceptance Criteria(边界 guard 段 — E18 OAuth provider 异常 toast):**

**Given** 用户点 GitHub/Google 登录(2.1 OAuth 登录段)
**When** OAuth provider down / email null / state 参数过期
**Then** toast 处理:provider down 友好提示“登录服务暂不可用”非裸异常;null-email 提示“provider 未返回邮箱”拒绝建账号;state 过期提示“登录已过期请重试”
**And** 不崩登录页不静默,异常分类提示用户可重试

#### Story 2.2: 画板归属与静态权限模型

As a 画板 owner,
I want 画板归属我与可管理 editor/viewer 角色,
So that 我控制谁能编辑或只看我的模型。
...
**Acceptance Criteria(边界 guard 段 — E20 owner transfer target 合法性):**

**Given** owner 调 POST /api/boards/{id}/transfer(new_owner_id)(2.2)
**When** new_owner_id 为 self / non-member / non-existent
**Then** validate target is member 且 !=self 且 exists,任一不满足返 400/403
**And** 防转让给自己(无意义)/转给非成员(无人接手)/转给不存在 user_id(孤儿画板)

#### Story 2.3: share_token 生成与轮换

As a 画板 owner,
I want 生成可轮换的只读分享链接,
So that 我能邀请他人查看模型且泄露后可撤销。
...
**Acceptance Criteria(边界 guard 段 — E21 token 轮换失效派生 session):**

**Given** owner 轮换 share_token(2.3 轮换段)
**When** viewer 持 active session(经旧 token 派生的 WS session)
**Then** invalidate rotated token 派生的 sessions(旧 token 立即失效,WS session 断开或降级)
**And** 防 viewer 用旧 token 持续访问(轮换语义是撤销旧访问权,非仅停新访问)

**Acceptance Criteria(边界 guard 段 — AR#4 viewer 身份模型决策):**

**Given** viewer 持只读 share_token 经 WS 连接(3.1)并出现于在线列表(3.2 需 {id,name,color})
**When** viewer 身份建模(决策)
**Then** viewer 获瞬时只读身份:临时 id 派生自 share_token,不建 users 行不持久化(非 AD-16 持久 session),在线列表显 {id,name=“viewer”/匿名,color}
**And** viewer 无写权限(3.3 viewer op 拒收),身份随 token 失效(E21)或断开结束
**And** 决策依据:viewer 是只读访客非持久用户,建 users 行致账号债;瞬时身份满足在线列表协议字段(3.2)

**Acceptance Criteria(边界 guard 段 — AR#12 share token 失效 404/403):**

**Given** viewer 持 share_token 访问画板(2.3)
**When** token invalid(不存在)/ expired(轮换失效,E21)/ 权限不足
**Then** 显 404(token 不存在)/ 403(权限不足/已失效)错误页 + 引导(提示“链接已失效,请联系 owner 获取新链接”非裸 404)
**And** 防 viewer 见裸 404/403 不知如何处理;错误页区分 invalid vs expired 引导不同(owner 轮换 vs 链接打错)

#### Story 2.4: 徽章绑账号持久化基座

As a 跨设备建模者,
I want 徽章记录绑账号持久化,
So that 我换设备登录仍见已解锁徽章。
...
**Acceptance Criteria(边界 guard 段 — E22 award FK check):**

**Given** award(user_id, badge_id) 写 user_badges(2.4 write API)
**When** badge_id 或 user_id non-existent
**Then** INSERT OR IGNORE 前 FK check(user_id 须存于 users / badge_id 须属内置徽章集),non-existent 拒绝返错误非静默 IGNORE
**And** 防 award 给不存在 user/badge 致孤儿记录;5.3 服务端判定调用前 user/badge 必然存在(防伪造),FK check 是兜底

**Acceptance Criteria(边界 guard 段 — AR#14 award API 调用面澄清):**

**Given** award() 供 Epic 5 服务端判定调用(2.4 write API)
**When** 澄清 award API 调用面(决策)
**Then** award API 仅 server 内部调用(server-to-server,5.3 服务端判定后调),不暴露 client HTTP 端点
**And** 因仅可信服务端可调,award write API 速率限制移除或降为内部调用量保护(无 client threat model);5.3“防客户端伪造”成立(客户端不可直接调 award)
**And** 决策依据:若暴露 client 则 5.3 防伪造弱化;仅 server 内部则限速无 threat model,与 AD-16 服务端判定一致
...
**Acceptance Criteria(边界 guard 段 — E14 OpLog 无界增长 compaction):**

**Given** SQLite 追加式 OpLog 持续增长(3.1 持久化段)
**When** OpLog 达 compaction/snapshot threshold
**Then** 触发 oplog compaction(旧 op 截断/归并到 snapshot,保留 snapshot+增量 oplog)
**And** 防无界增长致单节点 SQLite 膨胀(单节点 10 人规模,实现期标定阈值)

**Acceptance Criteria(边界 guard 段 — AR#3 origin_user_id relay ingress stamp):**

**Given** relay expose ops 订阅接口推送结构化 op(含 origin_user_id 字段)
**When** 客户端 CRDT op 经认证 WS session 到达 relay ingress
**Then** origin_user_id 由 relay 在 ingress 从认证 WS session stamp(非读客户端 Yjs op.origin)
**And** 忽略客户端 op.origin 字段(任何客户端可伪造 op.origin 瓦解 5.3 服务端徽章判定依赖的该字段,AD-16 防伪造)
**And** 5.3 徽章服务端判定读取的 origin_user_id 是 relay 权威 stamp 非客户端自报

**Acceptance Criteria(边界 guard 段 — AR#9 全量重同步 fallback 性能):**

**Given** yjs-go sync protocol 不达标 fallback 降级全量同步快照对齐 oplog(3.1 relay 段)
**When** 客户端重连触发全量重同步
**Then** 全量重同步频率/成本上限受控(防重连风暴:重连退避+全量同步并发上限),不破 NFR-PERF-3(P95 ≤100ms)
**And** 守卫:重连风暴(高频断/连)时全量同步节流/排队,防 10000 图元板每次重连触发全量同步压垮单节点 relay
**And** 全量同步是降级路径非稳态(达标 sync protocol 下走增量),性能上限为降级可接受非稳态 KPI

**Acceptance Criteria(边界 guard 段 — B-obs-2 op 延迟测量):**

**Given** NFR-PERF-3 协作延迟 P95 ≤100ms 有 KPI
**When** yjs-go relay 中转 op
**Then** relay 侧记录 op 延迟 metrics(ingress→egress 时戳),上报云平台内置监控(stdout)
**And** 测量方式与 P95 口径对齐(运维可查线上协作延迟达标)

**Acceptance Criteria(边界 guard 段 — E19 WS token 中途过期重连):**

**Given** active WS 连接(3.1,2.1 session token 中途过期)
**When** WS 鉴权 token 过期
**Then** graceful reconnect + refresh(WS 主动断开提示重连 / 客户端用 refreshed token 重连,非静默断开丢失编辑)
**And** 重连后 op replay(待处理 op 重放,NFR-REL-3)+ CRDT 状态对齐(增量/全量,AR#9);防 token 过期致编辑丢失

**Acceptance Criteria(边界 guard 段 — AR#5 REL-1 可用性):**

**Given** WS server(3.1)单节点云托管(AD-2/AD-18)
**When** 衡量服务可用性
**Then** 目标 99.9%(NFR-REL-1),运维监控 uptime + 故障恢复(进程重启 session 不丢,2.1 持久化)
**And** MVP 单节点无 HA,99.9% 靠进程级恢复 + Docker 重启策略;测量口径:月度可用时间占比

#### Story 3.2: 存在感知协议

As a 协作建模者,
I want 看到协作者的实时光标与选区,
So that 协作时有位置感知不冲突操作。
...
**Acceptance Criteria(边界 guard 段 — AR#5 REL-3 自动重连+op replay):**

**Given** 客户端 CRDT 集成(3.2)连接断开(网络抖动/服务端重启)
**When** 断线后
**Then** auto-reconnect(指数退避重连,NFR-REL-3)+ 重连后 op replay(待处理 op 重放,与 E19/AR#9 一致)
**And** 防 silent disconnect 丢编辑;replay 期间 UI 显“重连中”状态(1a.7 状态栏连接状态)

#### Story 3.3: 权限 runtime enforcement

As a 画板 owner,
I want viewer/editor 角色实时生效,
So that 权限变更即时阻止越权操作无窗口。
...
**Acceptance Criteria(边界 guard 段 — E13 stockValue NaN 跳过广播):**

**Given** 房主广播 [stockId,currentValue] 数组(3.4)
**When** 某 stockValue 为 NaN/Inf(熔断前瞬态或异常)
**Then** skip 该条广播(不广播 NaN 到客户端)或 halt 整步广播(触发 FR-SIM-6 熔断暂停)
**And** 防客户端渲染 NaN 致 sparkline/数值显示退化(与 1b.5 熔断边界协同)

#### Story 3.5: 房主迁移机制

As a 协作建模者,
I want 房主断开新房主无缝续跑,
So that 仿真不因房主掉线清零回退 t=0。
...
**Acceptance Criteria(边界 guard 段 — E12 host flapping grace+stand down):**

**Given** 房主心跳丢失窗口触发迁移(3.5)
**When** 原房主在 re-election 后重连(flapping,心跳抖动)
**Then** grace period(实现期标定,F17-host-migration-trigger)内不立即迁移,防心跳抖动误触发
**And** old-host 在 re-election 完成时 stand down(放弃房主身份,不再跑仿真/广播),防 split-brain(双房主同时跑仿真致仿真态分叉)
**And** grace period + stand down 保反指标房主迁移仿真清零=0(不因 flapping 反复迁移清零)

#### Story 3.6: 公式语义感知合并

As a 协作建模者,
I want 并发编辑公式不破坏语义,
So that 多人改同一公式合并后仍可解析。
...
**Acceptance Criteria(边界 guard 段 — E28 三路并发重冲突 AST merge order):**

**Given** 三路客户端并发编辑同一流量 formula AST 且触发重冲突(删子树幽灵编辑,FR-COLLAB-6 重冲突)
**When** CRDT 合并三路 update
**Then** 定义 multi-way merge order(按 clientID+clock 向量确定性排序依次两两合并)或 last-writer-wins(按 nid 单调计数器最大者胜)
**And** 合并后拓扑比较检测重冲突回退文本级 CRDT 降级(主 AC 分级回退不变),三路合并不丢字不破坏 AST 可解析性
...
**Acceptance Criteria(边界 guard 段 — AR#7b boards name 字段决策):**

**Given** 最近画板列表(4.1,最多 10 项)须渲染画板名
**When** 决策画板名存储(决策)
**Then** boards 表加 name 冗余列(CRDT boardMetadata 同步时 server stamp 入 boards.name),列表直接查关系表
**And** 决策依据:lazy-load 每板 CRDT 渲染名昂贵(10 板 × CRDT 反序列化);冗余列 server stamp 保列表查询 O(1);name 更新经 CRDT op → server stamp 同步 boards.name(最终一致)
...
**Acceptance Criteria(边界 guard 段 — E15 CRC32 碰撞非加密补结构 parse):**

**Given** CRC32 仅作传输完整性非加密认证(可能碰撞,4.3 剪贴板协议段)
**When** 粘贴 CRC32 校验通过(但 payload 碰撞畸形)
**Then** CRC32 后加结构 parse validation(字段类型合法/UUIDv4/fromId-toId 引用存在/无未知字段,任一不合规拒绝)
**And** 结构白名单是真实 guard 非 CRC32(防 CRC32 碰撞致畸形结构通过,反指标剪贴板畸形图元注入=0 由结构 parse 兜底)

**Acceptance Criteria(边界 guard 段 — E16 paste element count bounds):**

**Given** paste payload 含 element count
**When** count=0(空 payload)或巨量(超上限)
**Then** insert 前 enforce count bounds [1..N](N 实现期标定,防巨量图元压垮 CRDT/渲染)
**And** count=0 拒绝(空粘贴无意义),count>N 拒绝+toast“单次粘贴上限 N 图元”

**Acceptance Criteria(边界 guard 段 — B-perf-2 批量合并重绘突刺):**

**Given** 远端批量操作(如粘贴大量图元 E16 接近上限)到达瞬间
**When** 触发大范围脏矩形重绘
**Then** 批量操作合并瞬间帧率突刺为可接受瞬时掉帧(定性口径,类 F4 美学连续性非硬 KPI),不破稳态 NFR-PERF-1
**And** 显式声明合并突刺为可接受瞬时掉帧(非稳态帧率 KPI 违背);稳态帧率 KPI 不含批量合并瞬间
...
**Acceptance Criteria(边界 guard 段 — E17 new op 清 redo stack):**

**Given** 用户撤销后(undo)产生 redo stack 可重做
**When** 用户发起新 user op(新编辑/粘贴/删除等)
**Then** new user op 时 clear redo stack(废弃 redo 历史防 stale 重做)
**And** 防 undo 后新 op 使 redo stack stale(重做过时操作覆盖新状态),Y.UndoManager 默认语义对齐
...
**FRs covered:** FR-UI-6, FR-GAME-1, FR-GAME-2, FR-GAME-4(共 4,含 FR-GAME-2 split 主体)
...
**Acceptance Criteria(边界 guard 段 — E25 Web Audio autoplay 恢复):**

**Given** blip 音频(5.1 第 2 项 Web Audio API)+ 浏览器 autoplay policy(首次无用户手势前 audio context suspended)
**When** 首次用户手势(点按钮/键入)
**Then** resume AudioContext(首手势激活,非静默无声),后续 blip 正常触发
**And** 防 autoplay policy 致全程无声(用户已交互但 context 未 resume)

**Acceptance Criteria(边界 guard 段 — AR#11 prefers-reduced-motion):**

**Given** 前 8 项动效(5.1,含流量行进/glitch/呼吸辉光/输入火花)
**When** 用户系统 prefers-reduced-motion: reduce
**Then** 降级:减弱/禁非必要动效(行进动画减弱/glitch 降频/呼吸辉光静态/输入火花关),保核心建模语义可读性
**And** 防前庭功能障碍用户眩晕;降级不删核心信息(与 5.2 降级口径一致)

**Acceptance Criteria(边界 guard 段 — B-perf-1 行进动画 60FPS):**

**Given** 流量箭头行进动画(5.1 第 1 项,渲染循环驱动)
**When** 1000 流量图元全行进
**Then** 帧率 ≥ 60 FPS(NFR-PERF-1 动态口径,与 1a.5 静态口径互补)
**And** 或显式声明行进动画图元数上限为性能边界(若 1000 全行进不达 60FPS,标上限非降级失败)
...
**Acceptance Criteria(边界 guard 段 — AR#11 prefers-reduced-motion 第 9/10 项):**

**Given** 第 9/10 项(CRT 漂移/per-glyph 辉光,5.2)
**When** prefers-reduced-motion: reduce
**Then** 降级路径:删第 9/10 项(与 1a.2 spike 不可达降级口径一致,非回退 shadowBlur)
**And** reduced-motion 触发等同 spike 不可达降级,沉浸强化层缺失核心建模语义不降级
...
**Acceptance Criteria(边界 guard 段 — E24 halted sim 是否算“完成首模型”):**

**Given** 触发器(5)“完成首模型”= ≥1 存量+≥1 流量并成功跑一次仿真(5.3)
**When** sim 经 FR-SIM-6 熔断 halt(数值域熔断,非正常完成)
**Then** 决策:halted sim 不算“成功跑一次”(熔断是异常停非成功完成),不触发(5)徽章
**And** 区分:正常播完一段仿真(用户主动暂停或达设定终点)算“成功跑一次”触发;熔断 halt 不算;5.3 判定读 op 流 + 求解器终止原因(FR-SIM-6 halt 标志)区分

**Acceptance Criteria(边界 guard 段 — AR#13 多 tab/离板 badge push):**

**Given** 徽章达成服务端 push(5.3,经 AD-16 session WS 通道)
**When** 用户开多 tab(≤5,4.1)或不在任何板(离板)
**Then** push 到该 user 全部活跃 tab(≤5 WS 通道都收,非仅板 A);离板时无 WS 通道则降级 2.4 read 最终一致(下次进板/查徽章读 DB)
**And** 防 live toast 丢失(多 tab 仅板 A 收致其他 tab 无 toast);离板兜底 2.4 read 保最终一致(非永久丢)
...
**And** 里程碑 tab(FR-UI-7)回顾内容在本 story 实现(结构在 1a.12 重构 story defer 到此)
...
**Given** Story 5.4
**When** 执行
**Then** 依赖 5.3(徽章系统+总开关)+ 1a.12 重构 story(里程碑 tab 结构)
...
**AR#8 回收:inline B# 全文登记(原仅括注无索引,归入本清单可追溯):**

- **B2 rate-limit/滥用防护决策(MVP 单节点基线防护)**:award write API 速率限制(防 API 滥用刷 award 致查询/鉴权负载升,2.4)+ WS 连接速率限制(NFR-SEC-5 落地,防高频连/断 DoS 耗尽单节点 relay 资源,3.1);token bucket/固定窗口实现期选。MVP 单节点基线防护非规模化限流。inline @ Story 2.4/3.1。
- **B3 审计日志(owner 转让/角色变更)**:owner 转让(2.2)/角色变更审计追溯;MVP 10 人单节点规模审计非核心(纠纷可人工排查单节点日志/DB 直查),后续规模化时加。inline @ 来源标注。
- **B5 expose ops 订阅契约字段 spec 锁定折中**:relay push 结构化 op 非 raw Y.js update;安全字段(origin_user_id 防伪造 AD-16 + ts 时序判定)spec 锁防实施期漏,结构字段(type/payload)实现期 relay 与消费方对齐保灵活(B5 c 折中 / A1:极限1 全降级→安全字段漏 / 极限2 全保留→锁死 两难化解)。inline @ Story 3.1。
- **B10 空模型禁仿真触发**:无图元时禁用仿真触发(播放/暂停/单步/重置 disabled),防空模型跑仿真崩 1b.3。inline @ Story 1a.7。
- **B12 全员断线 cold restart 恢复**:全员断线无房主时 cold restart,1b 未就绪时允许重置 t=0 属降级路径非反指标违背(AR-2 修订:反指标 live-migration scope 仅含房主断开且他人在线续跑不含 cold restart)。inline @ Story 3.5 反指标 AC。
- **B13 切画板撤销栈独立**:切换画板时撤销栈独立(撤销作用于当前活跃画板非上次操作画板),Ctrl+Z 时活跃画板栈空 toast“当前画板无可撤销操作”,防用户预期“撤销跨画板粘贴”错位。inline @ Story 4.4。
- **B14 award 幂等**:重复 award 同徽章 INSERT OR IGNORE 不报错,UNIQUE(user_id, badge_id) 约束防重复插入。inline @ Story 2.4。
...
**AR#12 UX-state 决策(并入相关 story AC,非新增 UX-state story):**
...
> - 运行时错误(全局错误边界 + toast)→ Story 1a.1(全局 error boundary;worker crash E23/FR-SIM-6 熔断 toast 在 1b 域,本 story 提供全局兜底基座)
> - 404/403(share token 失效)→ Story 2.3(invalid/过期 token 错误页 + 引导重新获取)
