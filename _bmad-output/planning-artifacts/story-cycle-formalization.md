...
- gate: 零歧义 (AC 无多种解读) + 零遗漏 (epic AC 全覆盖) + 可执行 (dev 能直接做) + web research 显式记录 (step4 no-op 也算, 禁静默 skip — VS 门控拦截 CS step4 缺失, 不留到事后审计) + task↔CS 决策 一致性 (Tasks/Subtasks 行实现方向须与 CS 决策项逐条一致, 矛盾拦在 VS 不漏到 CR, 1a.7 教训: DS 按 T11 偏离 CS 决策#7 致 F-1-4 漏到 CR) + e2e spec 可跑性 gate (AC 含 e2e 时): VS 须核 e2e selector 可跑 - 1a.8 教训: CanvasView 纯 WebGL canvas 无 DOM overlay, property-panel.spec.ts selector mismatch 致 green-phase 不可行 7 test .skip() defer D4 归 1b; AC 含 e2e 时核渲染架构 (DOM 可断言 vs canvas-only), 拦 selector mismatch 不留 DS
...
- step8 mark complete 前 baseline diff review: `git diff <baseline_commit>..HEAD` 逐文件核, **须产出可审计逐文件核验表** (防 DS 虚假声明 - 1a.4 F9 抓 12+ false claims / 1a.7 偏离 CS 决策#7; #1 落地):
...
- **选项 B**: 手动检查清单:
  ...
  - task↔CS 决策 一致性 (Tasks/Subtasks 行实现方向须与 CS 决策项逐条一致, 矛盾拦在 VS 不漏到 CR, 1a.7 教训)
  - 约束引用 (AD/CAP 显式)
  - 测试标准 (TDD red-green)
  - web research 显式记录 (step4: 有新依赖记 version+why+breaking; 无新依赖记 no-op 引用基座 version 锁; 禁静默 skip)
  - 依赖标注 (前置 story/AD)
- **gate**: 零歧义 + 零遗漏 + 可执行 + web research 显式记录 + task↔CS 决策 一致性 (Tasks/Subtasks 须与 CS 决策项逐条一致, 矛盾拦在 VS)
