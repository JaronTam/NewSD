...
**AC-13**(A2 fold)— **Given** `RenderInstance` 契约(renderer.ts:24-30 现 5 字段) **When** 1a.3 扩展 **Then** `RenderInstance` 增加 `entityType`(stock/cloud/flow 枚举)/`zOrder`(number)/`rotation`(number,弧度)/`selected`(boolean)四字段;vertex shader 增加 `a_rotation` 旋转 quad corner;vertex shader 在 `selected` 时经 `effectiveLuma = a_lumaIdx + a_selected` 选更亮 atlas band 提升 luma 档(选中辉光;UV 在顶点计算, fragment 无法改采样 band, 故 AC-13 原措辞 "fragment shader" 修正为 "vertex shader" — 见 Dev Notes M1 备注;**不是** shadowBlur,CAP-11)。
...
- [x] **Task 3: VRAM 渲染器硬化(A1/A2/A4 fold)**(AC: 13, 14, 15)
  - [x] 3.1(A2)`renderer.ts:24-30` `RenderInstance` 增 `entityType`/`zOrder`/`rotation`/`selected` 四字段;`shaders.ts` vertex shader 增 `in float a_rotation` + 旋转 `cornerOf` 输出, 并经 `effectiveLuma = a_lumaIdx + a_selected` 在 selected 时选更亮 atlas band 提升 luma 档(选中辉光; UV 在顶点计算, fragment 不参与 band 选择, 死 `v_selected` varying 已删 — M1);`renderer.ts` 构造函数增 `a_rotation`/`a_selected` per-instance attrib + buffer;`CanvasView.tsx:118-139` `buildBootInstances` 更新 placeholder 填新字段默认值。
...
- [x] **Task 4: 存量渲染**(AC: 4, 6)
  - [x] 4.1 新增 `src/lib/render/elements.ts`——`stockToInstances(stock: Stock, simRunning: boolean): RenderInstance[]` builder:方框 `┌─┐│└┘`(经 `charToGlyphIdx`),居中 `name + (simRunning ? currentValue : initialValue) + units`;`units` 空时仅显数值。colorIdx = 0(stock cyan);`selected` 标志透传 `pushChar`, vertex shader 经 `effectiveLuma = a_lumaIdx + a_selected` 提档(AC-13; CPU 不再 bump `lumaIdx` — M1)。
...
- [x] **Task 7: 图元交互**(AC: 7)
  - [x] 7.1 `CanvasView.tsx` pointer 事件:拖拽 stock 移动(套 `snapToGrid` + `shouldSnap` 容差);调整大小(handle hit-test);点击选中(`selected` → mutation API,AC-14);双击编辑 name(inline input 或 modal)。
    - **CR followup (L9 落地)**: resize "调整大小" 由 handle hit-test 真正实现 — `elements.ts:resizeStock(stock, handle, wx, wy)` 纯函数 (4 角 NW/NE/SW/SE, 对角固定, `validateStockSize` ≥3 clamp AC-8/9) + `CanvasView` pointerDown handle 命中区 (selected stock, screen-space 12px, 先于 findElementAt) + movePan resize 分支 (套 `snapToGrid`/`shouldSnap` AC-1/2/3) + endPan 清除 + hover cursor (nwse/nesw) + draw 4 handle (`fillRect`/`strokeRect`, CAP-11 safe). Cloud 固定 6×3 不可 resize (AC-12). 测试: `elements.test.ts` +8 + `CanvasView.test.tsx` +4。
  ...
  - [x] 7.3 **CAP-11 守卫**:选中辉光经 vertex shader `effectiveLuma = a_lumaIdx + a_selected` 提档(AC-13; CPU 透传 `selected` 标志, 不 bump `lumaIdx` — M1),**严禁** `ctx.shadowBlur =`(runtime 禁,CAP-11;`cap11-shadowblur-guard.test.ts` 结构 grep + `CanvasView.test.tsx:234-273` runtime spy 双守卫会捕获违规)。
...
- **CAP-11(runtime shadowBlur 禁止)**:结构 grep 守卫 `cap11-shadowblur-guard.test.ts`(walk `src/`,allowlist = `lib/render/vram/glowAtlas.ts` 唯一)+ runtime spy `CanvasView.test.tsx:234-273`(Proxy mock ctx 记录 `shadowBlur` 赋值)。**1a.3 新增的 stock/cloud/element draw 代码、选中辉光、resize handle 等均不得引入 `.shadowBlur =`**;选中辉光经 vertex shader `effectiveLuma = a_lumaIdx + a_selected` 提档(AC-13; CPU 透传 `selected` 标志, 不 bump `lumaIdx` — M1)。两守卫会捕获任何违规 PR。
...
- **M1(AC-13 措辞修正 — CR followup)**:选中辉光经 **vertex shader** `effectiveLuma = a_lumaIdx + a_selected` 选更亮 atlas band 实现(atlas UV 在顶点计算, fragment 无法改采样 band), 故 AC-13 原 "fragment shader ... luma 档提升" 措辞修正为 "vertex shader"。实现: `elements.ts` 透传 `selected` 标志给 `pushChar`/`pushString`/`cloudToInstances`(CPU 不再 `baseLuma = selected ? 1 : 0` bump `lumaIdx`, `lumaIdx` 恒 0 base);`shaders.ts` 删除死 `v_selected` varying(vertex `flat out` + 赋值 + fragment `flat in` 三处);`renderer.ts` 已正确读 `RenderInstance.selected` 入 `selectedBuf`(无改);`CanvasView.tsx` cloud 分支接 `selectedId`。回归守卫: `shaders.test.ts` 断言 `v_selected` 不再出现, `elements.test.ts` 断言 selected 实例 `lumaIdx===0` + `selected:true`。
...
- **A2(RenderInstance 4 字段)**:`renderer.ts:24-30` 接口扩展;vertex shader `cornerOf` 输出乘旋转矩阵(`a_rotation` per-instance float);vertex shader `effectiveLuma = a_lumaIdx + a_selected` → luma 档 +1(clamp 到 `LUMA_LEVELS-1`; UV 在顶点计算, fragment 不参与 band 选择, 死 `v_selected` varying 已删 — M1);构造函数(renderer.ts:189-225)增 `a_rotation`/`a_selected` per-instance attrib + buffer(stride 调整);`scratchRotation`/`scratchSelected` scratch buffer + `reallocBuffers` 同步。`buildBootInstances`(CanvasView.tsx:118-139)填默认(rotation=0, selected=false, entityType=0)。
...
- **AC 逐条验证**:
  ...
  - AC-13: RenderInstance 增 entityType/zOrder/rotation/selected 四字段,vertex shader a_rotation + `effectiveLuma = a_lumaIdx + a_selected` 选更亮 atlas band 提档(UV 在顶点计算, fragment 无法改采样 band, "fragment shader" 措辞修正为 "vertex shader" — M1)。renderer.test.ts/shader.test.ts 契约断言。
