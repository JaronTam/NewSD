...
**实现模式裁定**: 单 PR(详见 §6 评估)。1a.7 = DOM UI chrome(顶部工具栏 + 底部状态栏)+ AR#11 a11y(键盘/色盲/对比度)+ 键盘 Delete/方向键补全。单一技术子系统(DOM chrome),AC 13 条 < 20 阈值,无 findings fold(snap-tolerance re-defer,见 §SDR#8)。走完整 story-cycle(CS->VS->DS->CR),一个 story 一个 PR。
...
**Then** 设 toolMode(lift 为 React state,见 SDR#4),按钮选中态高亮(边框/bg + 文字,非纯色)
...
**Given** 状态栏渲染(AC-8)
**When** 1a 单人模式
**Then** 激活:图元计数(elementStore.getElements().length)、FPS(Debug,perfProbe.getMetrics().fpsP95 实值,见 SDR#6)
...
**Given** 工具栏/状态栏(1a.7)
**When** 键盘操作
**Then** 全功能键盘可达:Tab 焦点流遍工具栏按钮/控件/状态栏;快捷键(F/S/C/V 切工具,既有 L815-844);Delete/Backspace 删选中(AC-3);方向键移动选中图元(见 SDR#5,1 world unit/press,snapToGrid 对齐,无选中 no-op)
...
**And** 文字对比度 WCAG AA(>=4.5:1 正文):--ns-fg #c9d1d9 on --ns-bg #0a0e14 ≈ 12.5:1 ✓;--ns-fg-dim #4a5568 on --ns-bg ≈ 2.6:1 ✗ FAIL AA(见 SDR#2,禁用于正文,仅装饰/非必要)
...
- [x] T3(AC-1, AC-8): AppShell 布局(详见 SDR#3)
...
- [x] T7(AC-4): toolMode lift 为 React state(详见 SDR#4)
...
- **命令式 DOM 更新**:既有 HUD(L462-477)经 render loop(drawRef.current)命令式设 textContent,非 React state(避免每帧重渲染 canvas)。1a.7 状态栏频繁字段(图元计数/FPS/缩放%)+ 缩放滑块/指示器沿用此模式(经 ref + render loop 设值)。用户交互态(toolMode/dt)用 React state(不频繁)。详见 SDR#4。
...
- tailwindcss 4.2 + @tailwindcss/vite(styles.css L1 `@import "tailwindcss" source(none)` + L2 `@source "../src"` + L7 @theme inline 映射 --color-_->--ns-_;utilities 可用于 layout 但 chrome 视觉样式走自定义类 + tokens,详见 SDR#2)
- lucide-react 0.575(可用但零使用,1a.7 不引入,ASCII 美学)
  无 breaking change 风险(纯增量 DOM chrome)。

### 域模型对账表

| Epic AC(epics.md)                          | 代码符号(源核验)                                                                                                                                                             | SDR       |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
...
| L485-487 占位(online/FPS/connection)       | FPS = perfProbe.getMetrics().fpsP95(L79/107,start() L42 自跑 rAF 采样,1a.7 首消费者);online "1"/连接 "本地" 占位;L487 "占位" 仅指 online+connection,FPS 实值(详见 SDR#6) | AC-9      |
...
- AC 数:13 < 20 阈值。
- findings fold:无(snap-tolerance re-defer 见 SDR#8;reverse-cr A/B/C/D/E 无 1a.7 UI chrome fold 项,见 §4 核查)。
- 结论:单 PR 走完整 story-cycle。若 DS step4 前发现 scope 过大,回退 sub-PR 须 DS step4 前决策 + 记 story(story-cycle L157)。

### CSSDR(§3.1 工具栏矩阵 / §3.2 状态栏矩阵 / §3.3 九项横切)
...
- **VS 显式记录**:VS 须记 §2.2 gate 4 项(零歧义/零遗漏/可执行/web research 显式)+ SDR 9 项 + AC 覆盖 epic L467-501 + §6 单 PR 评估 + 契约真实性核验(memory newsd-story-cycle-bmad-skill-invocation:1a.6+ VS 必须显式记录,禁静默 skip)。
...
- **不动**:camera.ts/store.ts/perf-probe.ts/types.ts(消费既有 API,不改);i18n.ts(死代码,1a.9 处理);tokens.css(消费既有 token,必要时加 --ns-fg-mid 见 SDR#2)。
...
| 核验项                                 | 结果 | 备注                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------------- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| §2.2 gate 零歧义(AC 无多种解读)        | PASS | SDR#6(FPS)epic L487 "FPS…显示占位" 字面含 FPS,但 L485 记 `FPS(Debug)` 暗示实字段;parenthetical 仅释 online+connection;perfProbe 已自跑 L730,实值零成本。解读可辩护。#5(1 world unit/press)明确;#7(disabled-stub)边界清晰。无歧义                                                                                                            |
...
| SDR 9 项(§3.3)                      | PASS | 逐项核(见下附 SDR 9 项明细表)                                                                                                                                                                                                                                                                                                                |
...
1. **SDR#6 FPS 解读张力**:epic L487 字面 "在线用户数/FPS/连接状态在 1a 单人模式下显示占位" 可读作 FPS 亦占位。CS 裁定 FPS 实值(L485 Debug 暗示+perfProbe 已自跑)可辩护但非唯一解读。DS 时如判应占位,fpsP95 fallback "-" 已覆盖(初始无样本即显 "-"),改动成本低。
...
零 blocker finding。2 advisory notes 为 judgment-call 记录,不阻塞 DS。CS 产出质量:AC 零歧义/零遗漏/可执行,web research 显式,SDR 9 项全 sound,契约真实性全核通过,单 PR 评估成立。
...
| F-1-4 / SDR#7  | 核验  | handleNew 非模态 confirm(`promptStore.confirm` Promise)+ `setElements([])` + 清选中 + 重置相机 `{0,0,16}`。**核验通过**                                                                                                                                                                                                |
