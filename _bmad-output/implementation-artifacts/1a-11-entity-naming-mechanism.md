...
- [ ] **AC-18(自动命名 skip-forward - rename 撞占据 canonical 名后自动跳号)** (a) Given store 含 stock_1/stock_2/stock_3(stockSeq=3) When updateElement(stock_2.id,{name:"stock_5"}) 手动改到未来 canonical -> createStock 不传 name ×3 Then 依次返回 stock_3(seq 4, taken 集 {stock_1,stock_5,stock_3} 无冲突✓)-> stock_4 -> stock_6(seq 6 因 5 被占, ++ 到 6). (b) Given cloud/flow 三 kind 各自独立循环(不跨 kind), createCloud/createFlow 同样 skip-forward. [SDR#14 新增, 保证自动命名永不撞用户手动改到 canonical 名的元素]
- [ ] **AC-19(updateElement name 类型收窄 - 非字符串拒绝)** Given store 含 stock (name:"A") When updateElement(id,{name:undefined}) 或 {name:null} 或 {name:42} Then throw "Name must be a string"(TypeError-style rejection) And 原 name 仍 "A". [SDR#11 补, `"name" in patch` 语义: undefined 值仍触发分支, 须 typeof 收窄 -- 防 String(undefined) 静默腐蚀]
...
> 每条 SDR 标分类(**[设计契约]**=真决策需 DS 实现 / **[保留不变量]**=勿动 / **[流程 meta]**=单PR/IR/e2e scope 非代码); 设计契约 SDR 含 (现状/目标/守卫) 三元, 守卫=AC# + 红测试断言(含"旧态消失"反向债拆除断言). Task 行 `gov: SDR#N` 引用见 Tasks 段(formalization §2.1/§2.2 追溯矩阵). 共 14 SDR(#14 为 CR Run 1 补).
...
14. **[设计契约] nextDefaultName skip-forward (rename->canonical 场景保护)**: 现状=1a.11 DS Run 1 落地的 nextDefaultName 单次 `seq++` + 拼字符串(store.ts L95-115), 与 SDR#4 assertNameAvailable(create-path)组合成"seq++ 后仍撞已占 canonical 名 -> throw"(rename stock_2 -> stock_5 后 createStock 到 seq=5 撞名); 目标=nextDefaultName 用 `do { seq++; candidate = <type>_<seq>; } while (taken.has(candidate))` 循环跳过被占 canonical 名, 保证自动命名"永不撞名, 永不 throw"契约(elements 有限 + seq 严格递增 -> 循环终止); 与 SDR#2/#12 关系=正交契约(#2 是单调 high-water + create/load 双路径; #12 是 rename 脱规范时 seq 不回退; #14 是 rename->canonical 时 seq 可跨号越过占位); 守卫=AC-18(rename stock_2 -> stock_5 后 createStock ×3 依次 stock_3/stock_4/stock_6)+ T2 refactor 红测试. [CR Run 1 F-1 root fix, gov ref T2]
...
#### CR Run 1 追加 (2026-07-16, 4 patch)

| 文件                                              | CR 补丁声明                                                                                                  | diff 实际                                                                                     | 一致? |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- | ----- |
| `src/lib/sd/store.ts` (nextDefaultName)           | F-1 skip-forward `do { seq++; ... } while (taken.has(candidate))` — SDR#14                                   | L95-120: switch 三 kind 各改 do/while 循环 + JSDoc 段 SDR#14 skip-forward 说明                | YES   |
| `src/lib/sd/store.ts` (updateElement)             | F-2 typeof guard `if (typeof patch.name !== "string") throw`                                                 | L264-276: `"name" in patch` 分支内加 typeof 收窄 + 抛 "Name must be a string"                 | YES   |
| `src/lib/sd/store.ts` (standalone createFlow doc) | F-3 JSDoc 从 "E11 duplicate-name warning" → "Name uniqueness (SDR#4/AC-1) assertNameAvailable 硬拒"          | L378-422: JSDoc 重写 step ③ + `@throws {Error}` for duplicate/empty                           | YES   |
| `src/lib/render/PropertyPanel.tsx`                | F-4 `.name ?? ""` → `.name` (SDR#5 name: string 契约传播) + nameInputRef revert 同步                         | L89 `defaultValue={selectedElement.name}` + L102-104 nameInputRef.current.value 同源 + 注释补 | YES   |
| `src/lib/sd/store.test.ts`                        | AC-18 skip-forward (2 test rename→canonical + cloud/flow 独立) + AC-19 typeof (3 test undefined/null/number) | L1035-1113: 5 新增 `it` (2 AC-18 + 3 AC-19), 全部 `// gov: AC-N + SDR#M + T-K` 追溯注释       | YES   |

CR patch 5 行, 4 补丁 (F-1/F-2/F-3/F-4), 5 test 新增 (AC-18 ×2 + AC-19 ×3); 零回归.
...
| 2026-07-16 | CR Run 1 完成 (orchestrator-direct): 3-layer review 12→7 root cause → A+X 裁定; 4 patch (F-1 nextDefaultName skip-forward + F-2 updateElement typeof guard + F-3 standalone createFlow JSDoc rewire + F-4 PropertyPanel `?? ""` removal); 1 dropped (F-7); 7 defer (F-5/6/8/9/10/11/12 → deferred-work.md); 新 SDR#14 skip-forward + AC-18 (2 test) + AC-19 (3 test); 594/594 vitest + tsc 0 errors; story record 回填完整                                                            | CC (bmad-code-review)  |
...
### Run 1 - 2026-07-16 (orchestrator-direct, ark-code-latest)
...
**执行模型**: ark-code-latest 后端 subagent 两轴皆崩 (同步 hunter prompt-too-long / 异步 hunter 越界改文件被 TaskStop 杀), orchestrator 自跑 Read/Grep/`git show <sha>:<path>` 3 层.
**基线**: `03dfee6` (DS 完成 commit) → CR 补丁终态 (本 Run 完成后 commit).

#### Layer 1 (Blind Hunter, unbiased scan)

execution: orchestrator 直接 grep/read store.ts / PropertyPanel.tsx / store.test.ts + `git show <sha>` 交叉核; 无 subagent.
findings: 12 (F-1..F-12).

#### Layer 2 (Contract/SDR truthfulness cross-check)

execution: 逐 SDR (14 条) 与代码/测试比对 truthfulness; SDR 段↔store.ts↔store.test.ts 三向对齐核.
findings: 已并入 F-1..F-12 (无独立 L2 新增).

#### Layer 3 (Cross-check + hollow-audit)

execution: 每 finding 复核 (a) 是否为设计取舍非缺陷 (b) 反证测试是否存在 (c) 是否 hollow (捏造 line/API); F-7 flowCreateWarning parallel `names.join(", ")` 经复核为合法 (parallel 记录展示), dropped.
findings: 11 confirmed (F-1..F-6, F-8..F-12), 1 dropped (F-7).

#### Root cause taxonomy (12 → 7)

R1 rename→canonical (F-1) / R2 typeof narrowing (F-2) / R3 SDR#4 rewire cleanup (F-3) / R4 SDR#5 fallback propagation (F-4) / R5 lessons not SDR-ized (F-8/F-11) / R6 i18n defer (F-6) / R7 perf micro (F-12); F-5/F-9/F-10 defer 层杂项.

#### Patch decision (A+X, user-approved 2026-07-16)

**D-root** (HIGH root fix, 2 items):

- **F-1** nextDefaultName skip-forward — store.ts L95-120 改 `do { seq++; candidate = ...; } while (taken.has(candidate));` + 新增 SDR#14 契约 + AC-18 (2 test) `// gov: AC-18 + SDR#14 + T2`.
- **F-2** updateElement typeof guard — store.ts L264-276 `if ("name" in patch) { if (typeof patch.name !== "string") throw ...; }` 补 undefined/null/number 拒绝 + 新增 AC-19 (3 test) `// gov: AC-19 + SDR#11 + T3`.

**D-cleanup** (LOW same-domain, 2 items):

- **F-3** standalone createFlow JSDoc rewire — store.ts L378-422 JSDoc 改 step ③ 从 "E11 duplicate-name warning" 到 "Name uniqueness (SDR#4/AC-1) assertNameAvailable 硬拒 empty/duplicate", 加 @throws for duplicate/empty.
- **F-4** PropertyPanel `?? ""` removal — PropertyPanel.tsx L89 `defaultValue={selectedElement.name}` (was `.name ?? ""`) + L102-104 `nameInputRef.current.value = selectedElement.name` + 注释 "SDR#5 guarantees name: string; no nullish fallback needed".

**Dropped** (Layer 3 复核): **F-7** flowCreateWarning parallel `names.join(", ")` — 合法 (parallel 展示多条记录), 非 name 歧义.

**Defer** (7 items → deferred-work.md `## From Story 1a.11 CR (Run 1, 2026-07-16)`):

- **F-5** leading-zero canonical name 边界 → 1a-13 (autosave hydrate 场景)
- **F-6** window.alert i18n 硬编码 → 1a.9 (i18n 单一调用点)
- **F-8** guard order 优化 (empty check 前置 collision) → 1a.12 polish
- **F-9** setElements 静默丢弃非 canonical seq 的行为文档 → 1a-13 (autosave 语义化时补文档)
- **F-10** selection reset comment 补 (1a.8 F-2 教训 SDR 化) → 1a.12 polish
- **F-11** it.skip 遗留断言 SDR 锚 → 1a.12 polish + 补 SDR
- **F-12** deriveSeq 3×O(N) → 1×O(N) perf → 1b (性能优化专项)

#### 验证 (post-patch)

- `git status --porcelain`: 3 files modified (store.ts / PropertyPanel.tsx / store.test.ts) + 2 docs (story record 本节 + deferred-work.md).
- `npx vitest run --run`: 594 passed | 1 skipped (preexisting) / 21 files — Delta +5 tests (AC-18 ×2 + AC-19 ×3), 零回归.
- `npx tsc --noEmit`: 0 errors.
- 全套件口径 [[newsd-e2e-attestation-full-suite-not-subset]]: 594 全套件 count 非子集.

#### Verdict

**CR PASS** (failed_layers = []) — 4 patch 应用 (F-1/F-2 root + F-3/F-4 cleanup), 1 dropped (F-7), 7 defer (F-5/6/8/9/10/11/12) 落 deferred-work.md, story record 回填完整 (Run 1 段/ SDR#14 / AC-18/19 / step8 追加 3 行 / Change Log 2026-07-16 行).

#### Risks & Boundaries (Run 1 post-patch)

- **AC-19 test regex i18n 耦合 (LOW)**: `store.test.ts` AC-19 三条断言用 `/must be a string/i` 匹配 `updateElement` 抛错 msg `"Name must be a string"` (store.ts L273). 现态匹配, `i` flag 容错大小写变体. 未来 i18n 化 msg (如中文 "名称必须为字符串") 时, 测试需同步改 pattern 或引入 error code 断言. 非 blocking; 落 1a.9 i18n 单一调用点整改时统一处理.
- **Boundary**: 4 patch scope 严格 = F-1/F-2/F-3/F-4 已列文件行; F-7 dropped 不动 flowCreateWarning; F-5..F-12 defer 不动. `git status --porcelain` 5 files 与 patch decision 表 1:1 对齐.
- **Boundary**: sprint-status 更新与 story code PR 分开 [[newsd-sprint-status-separate-from-story-pr]]; 本 Run 只 commit code + 2 docs (story record + deferred-work.md), sprint-status 变更等 PR 合并后独立 chore PR.
