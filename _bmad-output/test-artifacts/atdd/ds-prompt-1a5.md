...
1. **Story file** (THE authoritative spec): `_bmad-output/implementation-artifacts/1a-5-spatial-index-viewport-cull.md`
   - Parse ALL sections: Story, Acceptance Criteria (AC-1..AC-9), Tasks/Subtasks (6 tasks), Dev Notes, CS 决策 (11 items), Domain Model Reconciliation, Project Structure, References
...
- CS 决策: 11 items that MUST be followed exactly
...
3. Integrate dirty decision in `CanvasView.tsx` render loop:
   ...
   - **3-branch render decision** (CS 决策 #5):
   ...
   - **WebGL scissor out-of-scope** (CS 决策: additive glow + post-culling small visible set)
...
6. **WebGL scissor out-of-scope**: CS 决策 #5. No `gl.scissor()` usage. "Partial redraw" is at instance-build layer, not screen-pixel layer.

7. **NFR verification split** (CS 决策 #7): Playwright validates culling effectiveness (`buildInstances().length`), NOT absolute FPS. Absolute FPS via PerformanceProbe/RUM in real browsers.
...
### CS 决策 (11 items — MUST follow exactly)

These are in the story file Dev Notes → CS 决策. Every one must be implemented as specified, no deviation:
